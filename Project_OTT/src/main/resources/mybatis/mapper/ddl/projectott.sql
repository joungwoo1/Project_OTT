...
create table T_Eval(
	...
	descrim 	varchar(255) not null comment '영화,평가 구분',
...
create index idx_Movie_board on T_Movie(bb_id);


create table DEPT(
	DEPTNO		char(5) primary key, 부서번호
	DNAME 		varchar(10), 부서이름
	LOC			varchar(5) 
);

create table EMP(
	DEPTNO		char(5) primary key, 부서번호
	ENAME		varchar(10), 직원이름
	JOB			varchar(10), 
	SAL			varchar(5)
); 

ALTER TABLE DEPT MODIFY LOC VARCHAR(10)

	
	WHERE detp,(
	(select *
		FROM emp
		where DEPTNO IN ('X20','P30')) rEmp
	DEPT.no = rEmp.deptno


select  emp.deptno, emp.ename, DEPT.deptno, DEPT.dname
from DEPT, emp
...
values('00001', '학원', '일/공부');
insert into DEPT(DEPTNO, DNAME,LOC)
values('00002', '직장', '일');
insert into DEPT(DEPTNO, DNAME,LOC)
values('00003', '학교', '공부');
insert into EMP(DEPTNO, ENAME, JOB, SAL)
values('00001', '홍길동', '교사', 'abc');
insert into EMP(DEPTNO, ENAME, JOB, SAL)
values('00002', '김동', '학생', 'ㄱㄱㄱ');

insert into DEPT(DEPTNO, DNAME,LOC)
values('X20', '333', '일/공3부33');
insert into DEPT(DEPTNO, DNAME,LOC)
values('P30', '112', '일/공부22');
insert into DEPT(DEPTNO, DNAME,LOC)
values('00001', '학원', '일/공부');

insert into EMP(DEPTNO, ENAME, JOB, SAL)
values('X20', '김동1', '학생', 'ㄱㄱㄱ');
insert into EMP(DEPTNO, ENAME, JOB, SAL)
values('P30', '김동', '학생22', 'ㄱㄱㄱ333');
